...
%rosinit
cd('/home/vise/Universita/Autonomous_vehicles/Assignments/Assignment_2/ExtraA/ListFromPython')
...
% Select critical radius around waypoints

r=0.05;

% select tolerances on the reaching of the waypoints
...
waypoints=out.waypoints(end,:);

if waypoints(35)==1001
    N=waypoints(36);
    waypoints=reshape(waypoints,[3 12])';
    waypoints(N+1:end,:)=[];
else
    N=12;
    waypoints=reshape(waypoints,[3 12])';
...
for ii=1:50:length(time)
    ...
    % if ii==1 || ii==901 || ii==1861 || ii==4201
